...
export function Sidebar({ className }: ISidbar) {
  ...
  const { rooms } = useGetRooms(user.user?.id);
            ...
            {rooms.map((room, idx) => (
              ...
              >
                <Avatar>
                  ...
                  ></AvatarImage>
                  <AvatarFallback className="">{`${room.consultor.firstName[0].toUpperCase()}${room.consultor.lastName[0].toUpperCase()}`}</AvatarFallback>
                </Avatar>
